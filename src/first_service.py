...
class FirstService:
    ...
    def _first_part(self, ont_base, te_ont_bl, is_plus_strain):
        ...
        raw_subseq_filename = (
            self.config.first_subseq_path / f"raw_{basename}_subseq.fasta"
        )
        ...
        te_ont_bed = te_ont_bl[
            ["sseqid", start_field, end_field, "qseqid"]
        ].sort_values(by=["sseqid", start_field, end_field])
        ...
        merged = pd.read_csv(
            ...
            sep="\t",
            header=None,
            names=["sseqid", "sstart", "send", "qseqid"],
        )
        filtered_bed = merged[(merged.send - merged.sstart) > self.config.from_te]
        Helpers.save_df(
            pd.DataFrame({"id": filtered_bed["sseqid"].unique()}), txt_filename
        )
        os.system(f"seqtk subseq {fasta_file} {txt_filename} > {raw_subseq_filename}")
    ...
    def first_part(self):
        for ont_base in self.config.ont_bases:
            ...
            self._first_part(
                ont_base, te_ont_bl[te_ont_bl.sstart <= te_ont_bl.send], True
            )
            self._first_part(
                ont_base, te_ont_bl[te_ont_bl.sstart > te_ont_bl.send], False
            )
    ...
    def create_subsequent_fasta(self, fasta, bed_df, filename):
        ...
        for bed_rec in bed_df.itertuples():
            ...
            left_start = (
                te_start - self.config.flanks_len
                if te_start - self.config.flanks_len > 0
                else 0
            )
            left_end = te_start
            ...
            right_start = te_end
            right_end = (
                te_end + self.config.flanks_len
                if te_end + self.config.flanks_len < len(record)
                else len(record)
            )

            left_subseq_record = record[
                left_start:left_end
            ]  # For the full record (with header)
